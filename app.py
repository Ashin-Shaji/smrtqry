...
db_name = st.text_input("Enter Database Name", value='dabase')

if db_name:
    create_db_button = st.button("Create/Connect to Database")
    if create_db_button:
      ...
      if st.session_state.conn:
          ...
          with st.form("employee_form"):
              ...
              existing_tbl_name = st.selectbox("Or Select Existing Table", [""] + [table[0] for table in tables])
          ...
          table_name = st.selectbox("Select Table Name to View Data", [table[0] for table in tables], index=0)
          ...
          if table_name:
              ...
              if st.button("Generate SQL Query"):
                  ...
                  # prompt = f"your only task is to just give only the proper clean sql query after understanding the input without giving unnecessary keywords, use the following schema data to create proper sql queries\n{schema_info}"
                  ...
                  # prompt = f"your only task is to just give only the proper clean sql query without any digit before the output and without giving unnecessary keywords after understanding the input, use the following data to create proper sql queries\n{df_string}"
                  ...
                  try:
                      ...
                      # t = st.selectbox("Do you want to save the result to CSV?", ("Yes", "No"),index=None)
                      # if t == "Yes":
                      #     result_df.to_csv('newss.csv')
                      #     st.success("Result saved to new.csv")
                      # elif t == "No":
